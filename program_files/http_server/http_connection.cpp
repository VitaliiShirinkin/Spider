...
HttpConnection::HttpConnection(tcp::socket socket, Search& search)
	: socket_(std::move(socket)), search_(search)
{
}
...
void HttpConnection::createResponsePost()
{
	if (request_.target() == "/")
	{
		...
		std::vector<std::string> searchResult = search_.searchEngine(utf8value);
		...
		else
		{
			beast::ostream(response_.body())
				<< "<p>"
				<< "No results found. Refresh the page and try another query."
				<< "</p>";
		}
